...
function AdCard({ ad, onDelete }) {
  return (
    <div className="flex flex-col gap-4 rounded-xl border border-ink-200 bg-white p-4 shadow-card sm:flex-row sm:items-center">
      <div className="h-28 w-full shrink-0 overflow-hidden rounded-lg bg-ink-100 sm:h-16 sm:w-32">
        <img src={ad.image_url} alt={ad.title} className="h-full w-full object-cover" />
      </div>

      <div className="flex min-w-0 flex-1 flex-col gap-1">
        <span className="truncate text-sm font-semibold text-ink-900">{ad.title}</span>
      ...
      <div className="flex items-center gap-3 sm:flex-col sm:items-end sm:gap-1.5">
        <span className="text-xs text-ink-500">Prioridade {ad.priority}</span>
        <AdStatusBadge ad={ad} />
      </div>
