    ...
    <div className="flex flex-col gap-5">
      <div className="grid grid-cols-3 gap-3">
        {['mobile', 'desktop', 'tablet'].map((key) => {
          ...
          return (
            <div key={key} className="flex flex-col items-center gap-1.5 rounded-lg border border-gray-200 p-4">
              <Icon className="h-5 w-5 text-brand-600" />
              <p className="text-lg font-semibold text-gray-900">{pct}%</p>
              <p className="text-xs text-gray-500">{meta.label}</p>
              <p className="text-[11px] text-gray-400">{formatNumber(views)}</p>
            </div>
          )
        })}
      </div>
        ...
        <div>
          ...
          <ul className="flex flex-col gap-1.5">
            {byOs.slice(0, 4).map((row) => (
              <li key={row.os} className="flex items-center justify-between text-gray-600">
                <span className="truncate">{row.os}</span>
                <span className="shrink-0 font-medium text-gray-900">{formatNumber(row.views)}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="flex flex-col gap-1.5">
            {byBrowser.slice(0, 4).map((row) => (
              <li key={row.browser} className="flex items-center justify-between text-gray-600">
