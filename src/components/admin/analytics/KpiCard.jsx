...
function Trend({ growth }) {
  if (growth === undefined) {
    return <span className="inline-flex items-center rounded-full bg-ink-100 px-2 py-0.5 text-xs font-medium text-ink-500">Novo</span>
  }

  const isUp = growth > 0.05
  const isDown = growth < -0.05
  const Icon = isUp ? TrendingUp : isDown ? TrendingDown : Minus
  const tone = isUp ? 'text-green-600 bg-green-50' : isDown ? 'text-red-600 bg-red-50' : 'text-ink-500 bg-ink-100'
  ...
  if (loading) {
    return (
      <div className="flex flex-col gap-3 rounded-xl border border-ink-200 bg-white p-5 shadow-card">
        ...
        <div className="h-8 w-20 animate-pulse rounded bg-ink-100" />
  ...
  return (
    <div
      className={`flex flex-col gap-2 rounded-xl border p-5 shadow-card ${
        highlight ? 'border-brand-200 bg-brand-50' : 'border-ink-200 bg-white'
      }`}
      title={hint}
    >
      ...
      <p className={`text-2xl font-semibold ${highlight ? 'text-brand-700' : 'text-ink-900'}`}>
        {formatNumber(value)}
      </p>
      <Trend growth={growth} />
    </div>
