  ...
  return (
    <div className="flex flex-col gap-12" aria-hidden="true">
      ...
      <div className="grid gap-10 lg:grid-cols-3 lg:gap-12">
        <div className="flex flex-col divide-y divide-ink-100 lg:col-span-2">
          {Array.from({ length: 6 }).map((_, index) => (
            <RowSkeleton key={index} imageClassName="h-20 w-20 sm:h-24 sm:w-32" className="py-4" />
          ))}
        </div>
        <div className="flex flex-col gap-4">
          {Array.from({ length: 5 }).map((_, index) => (
            <div key={index} className="flex items-center gap-3">
              <div className="h-8 w-7 shrink-0 animate-pulse rounded bg-ink-100" />
